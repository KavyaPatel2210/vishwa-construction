...
export default function EditInvoice() {
  ...
  // Original invoice info (read-only display)
  ...
  const [form, setForm] = useState({
    ...
    status: 'Pending',
    notes: '',
    categoryOfService: 'Civil Construction Work',
    gstEnabled: false,
    gstPercent: 18,
  });
  ...
  useEffect(() => {
    invoiceService.getById(id)
      .then(res => {
        ...
        // Block editing of paid invoices
        if (inv.status === 'Paid') {
          toast.error('Paid invoices cannot be edited. Mark it as Pending first.');
        ...
        setForm({
          ...
          items: inv.items?.length > 0
            ? inv.items.map(i => ({ description: i.description, amount: String(i.amount) }))
            : [{ ...EMPTY_ITEM }],
          status: inv.status || 'Pending',
          notes: inv.notes || '',
          categoryOfService: inv.categoryOfService || 'Civil Construction Work',
  ...
  const removeItem = (idx) => {
    if (form.items.length === 1) return toast.error('At least one item is required');
    setForm(p => ({ ...p, items: p.items.filter((_, i) => i !== idx) }));
  };

  const totalAmount = form.items.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
  const gstAmt = form.gstEnabled ? (totalAmount * (parseFloat(form.gstPercent) || 0)) / 100 : 0;
  const grandTotal = totalAmount + gstAmt;

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (form.items.some(i => !i.description.trim()))
      return toast.error('All work descriptions are required');
    if (form.items.some(i => !i.amount || isNaN(i.amount)))
      return toast.error('All amounts must be valid numbers');
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generatePDF('invoice-preview',
        `Invoice-${updatedInvoice.customerName}-Bill${updatedInvoice.billNumber}.pdf`);
    ...
    <MainLayout>
      <div className="max-w-3xl mx-auto">
        ...
        <button
          ...
          className="flex items-center gap-2 text-gray-600 dark:text-slate-400 hover:text-primary-600 mb-5 text-sm font-medium"
        >
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
              Bill #{originalInvoice?.billNumber} · {originalInvoice?.customerName}
            </p>
          </div>
          {/* Read-only bill number badge */}
          <div className="bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-xl px-4 py-2 text-center">
            <p className="text-xs text-primary-600 dark:text-primary-400 font-medium">Bill Number</p>
            <p className="text-xl font-bold text-primary-700 dark:text-primary-300">
              #{originalInvoice?.billNumber}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="card p-5">
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl mb-4">
              <div className="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-primary-700 dark:text-primary-400 font-bold flex-shrink-0">
                {originalInvoice?.customerName?.[0]?.toUpperCase()}
              </div>
              <div>
                <p className="font-semibold text-gray-900 dark:text-white">{originalInvoice?.customerName}</p>
                {originalInvoice?.customerAddress && (
                  <p className="text-xs text-gray-500 dark:text-slate-400">{originalInvoice.customerAddress}</p>
                )}
                {originalInvoice?.customerPan && (
                  <p className="text-xs text-gray-500 dark:text-slate-400">PAN: {originalInvoice.customerPan}</p>
                )}
              </div>
              <span className="ml-auto text-xs text-gray-400 dark:text-slate-500 italic">Cannot be changed</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className="form-input" value={form.categoryOfService} onChange={setField('categoryOfService')} />
              </div>
            </div>
          </div>
              ...
              {form.items.map((item, idx) => (
                <div key={idx} className="flex gap-3 items-start">
                  ...
                  <button
                    ...
                    className="btn-danger p-2 mt-1 flex-shrink-0"
                    title="Remove row"
                  >
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
      ...
      <Modal
        isOpen={showPreview}
        onClose={() => { setShowPreview(false); navigate(`/invoices/${id}`); }}
        size="xl"
      ...
      >
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          <button
            onClick={() => { setShowPreview(false); navigate(`/invoices/${id}`); }}
        ...
        <div className="overflow-auto max-h-[70vh] border border-gray-200 rounded-xl">
          {updatedInvoice && (
            <InvoicePreview invoice={updatedInvoice} contractor={user} />
          )}
        </div>
      </Modal>
    </MainLayout>
